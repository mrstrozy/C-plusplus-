...
int main(int argc, char** argv) {
   ...
   const string outfileName = infileName + ".out";


   //Create output file in "out" mode, which zaps previous contents
   //To preserve and append after previous contents, use ios_base:app instead

   ofstream outfile(outfileName.c_str(), ios_base::out);
   if (! outfile.is_open()) {
      cerr << "Unable to create and stat output file " << outfileName
           << endl;
      return(1);
   }
   ...
   //BECAUSE we want 2 separate copies, we use C++ VALUES, not pointers!
   ...
   string temp;
   while (numItems < n && infile >> temp) {           //see "Fubar?" note
   ...
   for (int i = 0; i < numItems; i++) {
      outfile << itemsCopy1[i] << endl;
   }
   infile.close();   //could also do delete(infilep) and delete(outfilep),
   outfile.close();  //which (should!) call close on the files.
   delete(timer);
...
/** Vector-Value Note: Treating vectors by-value, rather than by-pointer
    ...
    heap-object pointer can make the whole "HiResTimer" class simpler...if
